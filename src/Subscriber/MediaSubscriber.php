...
class MediaSubscriber implements EventSubscriberInterface
{
    ...
    public function onMediaLoaded(EntityLoadedEvent $event): void
    {
        ...
        // getting config
        $pluginConfig = $this->systemConfigService->get('ImageEngineCdn.config');
        ...
        if (method_exists($event->getContext()->getSource(), 'getSalesChannelId')) {
            ...
            if (!($this->systemConfigService->get('ImageEngineCdn.config.isActive', $salesChannelId))) {
                return;
            }
            $pluginConfig = $this->systemConfigService->get('ImageEngineCdn.config', $salesChannelId);
        }
        ...
        if ($request) {
            ...
            $originalDomain = $this->systemConfigService->get('ImageEngineCdn.config.shopDomain');
